            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    if (c.a > 0.5) {
                        ...
                        // MARKED pixel: relax the inherited target toward the min of
                        // neighboring marked/gap targets. Without this, each pixel
                        // freezes the first target its front delivered, tiling the
                        // region with hard plateaus at front-collision seams.
                        float relaxed = c.r;
                        for (int dy = -1; dy <= 1; dy++) {
                            for (int dx = -1; dx <= 1; dx++) {
                                if (dx == 0 && dy == 0) continue;
                                ...
                                if (n2.a > 0.5 && n2.b < 0.995) relaxed = min(relaxed, n2.r);
                            }
                        }
...
// ============================================================================
const MOEBIUS_DEBUG_VERSION = 'FG-SUB rimdepth v3.1 (interior flood plug)';
...
function exportDebugContactSheet() {
    try {
        ...
        const stamp = [
            ...
            'cam(' + cam.x.toFixed(3) + ', ' + cam.y.toFixed(3) + ', ' + cam.z.toFixed(3) + ') | view=' + dbgSel + ' | bgBias=' + bias + ' | fgThresh=' + thr + ' | fgReach=' + reachStamp + ' | fgSubRan=' + fgOk
        ];
        ctx.fillStyle = '#ff8';
...
function exportSDBundle() {
    try {
        ...
        if (typeof srcBandTargetA !== 'undefined' && srcBandTargetA && bgDepthTarget && bgColorTarget) {
            ...
            addFile('src_bg_depth_completed.png', bgDepthTarget.texture, null, 0);
...
let bgMorphMaterial = null; // grayscale erode/dilate for the depth opening
    ...
    try {
        ...
        if (!srcBandSeedMaterial) {
                ...
                fragmentShader: `
                    ...
                    void main() {
                        ...
                        for (int dy = -1; dy <= 1; dy++) {
                            for (int dx = -1; dx <= 1; dx++) {
                                if (dx == 0 && dy == 0) continue;
                                minNbr = min(minNbr, texture2D(tSrcDepth, vUv + vec2(float(dx), float(dy)) * u_texelSize).r);
                            }
                        }
                        float step_ = d - minNbr;
            ...
            bgColorSeedMaterial = new THREE.ShaderMaterial({
                uniforms: { tColor: { value: null }, tBand: { value: null } },
                vertexShader: vs,
                fragmentShader: `
                    uniform sampler2D tColor;
                    uniform sampler2D tBand;
                    ...
                    void main() {
                        vec4 b = texture2D(tBand, vUv);
                        if (b.a > 0.5) { gl_FragColor = vec4(0.0); }
        ...
        if (pullPyramidTargets.length >= 2 && pullMaterial && pushMaterial) {
            ...
            bgColorSeedMaterial.uniforms.tBand.value = srcBandTargetA.texture;
        ...
        mat.uniforms.displacementMap.value = bgDepthTarget.texture;
    ...
    if (debugView === 'plug_error') {
        ...
        if (!window._plugErrMaterial) {
            window._plugErrMaterial = new THREE.ShaderMaterial({
                uniforms: { tMask: { value: null }, tDepthBG: { value: null }, tColor: { value: null }, u_scale: { value: 8.0 } },
                vertexShader: `varying vec2 vUv; void main() { vUv = uv; gl_Position = vec4(position, 1.0); }`,
                fragmentShader: `
                    uniform sampler2D tMask;    // FG-only classification: gap => expected plug depth in R
                    ...
                    void main() {
                        ...
                        if (dbg.a < 0.5) {
                            // Plug MISSING here: BG layer produced no depth
                            gl_FragColor = vec4(1.0, 0.0, 1.0, 1.0);
                            return;
                        }
                        float err = abs(dbg.r - m.r) * u_scale;
                        // green (perfect) -> yellow -> red (off by >= 1/u_scale)
