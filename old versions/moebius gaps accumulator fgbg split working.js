...
/**
 * Renders the Texture UV coordinates of the scene geometry into uvMapRenderTarget.
 * This is required for accurate Texture Space accumulation.
 */
function renderUVMap() {
    if (!uvMapRenderTarget || !scene || !camera) return;

    // Store original materials and create temporary UV materials
    const originalMaterials = new Map();
    let uvMaterialsCreated = false;

    // Disable gap detection globally before rendering the UV map
    ...
    scene.traverse((object) => {
        if (object.isMesh && object.material && object.material.isShaderMaterial) {
            // Skip the atlas mesh itself if it exists
            if (object === infillAtlasMesh) return;

            const uvMaterial = createUVCaptureMaterial(object.material);
            if (uvMaterial) {
                originalMaterials.set(object, object.material);
                object.material = uvMaterial;
                uvMaterialsCreated = true;
            }
        }
    });

    if (!uvMaterialsCreated) {
        renderer.setRenderTarget(uvMapRenderTarget);
        renderer.clear();
        return;
    }

    // Render the scene into the UV Map Target
    renderer.setRenderTarget(uvMapRenderTarget);
    renderer.clear();
    renderer.render(scene, camera);

    // Restore original materials and dispose of temporary ones
    originalMaterials.forEach((material, object) => {
        if (object.material !== material) {
            object.material.dispose(); // Dispose the temporary UV material
...
/**
 * Initializes or updates the static mesh used to display the baked infill atlas.
 * CRITICAL FIX: Clones the geometry and material from the primary media layer 
 * and uses the VTF-compatible depth map for seamless alignment and displacement.
 */
function initializeInfillAtlasMesh() {
    // Ensure the atlas targets exist. Check the VTF target.
    if (!infillAtlasTarget_Color || !infillAtlasTarget_Depth_VTF) {
        return;
    }

    // Identify the primary media layer (the source geometry)
    const primaryLayer = mediaLayers.find(l => l.mesh && l.mesh.geometry && l.mesh.material);

    if (!primaryLayer) {
        // If no primary layer exists, we cannot create the atlas mesh. Remove existing if necessary.
        if (infillAtlasMesh) {
            console.log("Removing Infill Atlas Mesh (No primary layer found).");
            if (scene) scene.remove(infillAtlasMesh);
            infillAtlasMesh.geometry.dispose();
            if (infillAtlasMesh.material) infillAtlasMesh.material.dispose();
            infillAtlasMesh = null;
        }
        return;
    }

    const sourceGeometry = primaryLayer.mesh.geometry;
    const sourceMaterial = primaryLayer.mesh.material;

    // Helper to update textures and disable gaps on the material
    const configureAtlasMaterial = (material) => {
        if (!material || !material.uniforms) return;

        // Update Color Texture
        if (material.uniforms.map) {
            material.uniforms.map.value = infillAtlasTarget_Color.texture;
        } else if (material.uniforms.tDiffuse) {
            material.uniforms.tDiffuse.value = infillAtlasTarget_Color.texture;
        } else if (material.uniforms.rgbTexture) { // Handle "separated" mode
            material.uniforms.rgbTexture.value = infillAtlasTarget_Color.texture;
        } else if (material.uniforms.videoTexture) { // Handle "sidebyside" mode
            material.uniforms.videoTexture.value = infillAtlasTarget_Color.texture;
        }
        
        // Update Depth Texture (Use VTF-compatible depth)
        if (material.uniforms.depthMap) {
            material.uniforms.depthMap.value = infillAtlasTarget_Depth_VTF.texture;
        } else if (material.uniforms.tDepth) {
            material.uniforms.tDepth.value = infillAtlasTarget_Depth_VTF.texture;
        } else if (material.uniforms.displacementMap) { // Handle "image" mode
             material.uniforms.displacementMap.value = infillAtlasTarget_Depth_VTF.texture;
        } else if (material.uniforms.depthTexture) { // Handle "separated" mode
             material.uniforms.depthTexture.value = infillAtlasTarget_Depth_VTF.texture;
        }

        // CRITICAL: Disable all gap generation on the atlas material
        material.uniforms.u_useDepthGrad.value = false;
        material.uniforms.u_useSobel.value = false;
        material.uniforms.u_useLuma.value = false;
        material.uniforms.u_useChroma.value = false;
        material.uniforms.u_useCrease.value = false;
        material.uniforms.u_useCurvature.value = false;
        material.uniforms.u_useUVStretch.value = false;
        material.uniforms.u_useGrazingAngle.value = false;
        
        material.needsUpdate = true;
    };

    if (infillAtlasMesh) {
        // Check if the existing mesh geometry matches the current primary layer geometry
        if (infillAtlasMesh.userData.sourceGeometryId === sourceGeometry.uuid) {
            // Geometry matches, just update the material textures
            configureAtlasMaterial(infillAtlasMesh.material);
            return;
        } else {
            // Geometry has changed (e.g., media source changed). Dispose and recreate.
            console.log("Recreating Infill Atlas Mesh (Geometry changed).");
            if (scene) scene.remove(infillAtlasMesh);
            infillAtlasMesh.geometry.dispose();
            if (infillAtlasMesh.material) infillAtlasMesh.material.dispose();
            infillAtlasMesh = null;
    ...
    // --- Create New Mesh (Cloning from Source) ---
    
    // 1. Clone Geometry
    const geometry = sourceGeometry.clone();

    // 2. Clone Material
    // Cloning the ShaderMaterial ensures all uniforms (intrinsics, displacement settings) match the foreground.
    const material = sourceMaterial.clone();
    
    // 3. Configure the cloned material (swap textures, disable gaps)
    configureAtlasMaterial(material);

    // 4. Create Mesh
    infillAtlasMesh = new THREE.Mesh(geometry, material);
    infillAtlasMesh.renderOrder = 0; // Ensure it renders behind the foreground layers
    infillAtlasMesh.visible = useStaticInfillAtlas;
    
    // Store the source geometry ID to detect changes later
    infillAtlasMesh.userData.sourceGeometryId = sourceGeometry.uuid;

    if (scene) {
        scene.add(infillAtlasMesh);
    }
    console.log("Infill Atlas Mesh initialized (Cloned Geometry/Material, VTF Depth).");
}
...
function resizeRendererAndTargets(width, height) {
    ...
    // NEW: Ensure atlas mesh is updated after targets resize
...
function initializeSceneAndRenderer() {
    ...
    // NEW: UV Map Render Target (For Texture Space Accumulation)
    // Must be high precision (HalfFloat) and have a depth buffer.
    uvMapRenderTarget = new THREE.WebGLRenderTarget(renderer.domElement.width, renderer.domElement.height, {
        minFilter: THREE.NearestFilter, // Use Nearest to avoid interpolating coordinates at edges
        magFilter: THREE.NearestFilter,
        format: THREE.RGBAFormat,
        type: THREE.HalfFloatType,
        depthBuffer: true, // CRITICAL: Enable depth buffer to handle occlusion correctly
        stencilBuffer: false
    });
...
// ===================================================================
// START: BUG FIX (Revert layerMaskMaterial to use Hardware Depth)
// ===================================================================
// REPLACE the existing 'layerMaskMaterial' definition with this one.
    layerMaskMaterial = new THREE.ShaderMaterial({
        uniforms: {
            tDepth: { value: null }, // From sceneRenderTarget.depthTexture
            // REMOVED: tNormalizedDepthSource
        ...
        fragmentShader: `
            ...
            void main() {
                // REVERTED: Use rawHardwareDepth from tDepth (sceneRenderTarget)
                ...
                // Compare hardware depth to hardware split point
...
// ===================================================================
// START: BUG FIX (Fix 'Show Depth' Debug View)
// ===================================================================
// REPLACE the existing 'debugDepthMaterial' definition with this one.
    debugDepthMaterial = new THREE.ShaderMaterial({
        uniforms: {
            tDepth: { value: null }, // This will receive the 8-BIT SOURCE TEXTURE
            // REMOVED: Linearization uniforms
            u_depthPeekActive: { value: false }, // Default to false
            u_depthPeekValue: { value: depthPeekValue },
            u_depthPeekTolerance: { value: depthPeekTolerance },
        },
        vertexShader: `varying vec2 vUv; void main() { vUv = uv; gl_Position = vec4(position, 1.0); }`,
        fragmentShader: `
            ...
            void main() {
                // Read the 8-bit normalized depth directly
                float normalizedDepth = texture2D(tDepth, vUv).r;

                // Base color is the normalized depth
                vec4 finalColor = vec4(vec3(normalizedDepth), 1.0);

                // Show red peek highlight
                if (u_depthPeekActive) {
                    if (abs(normalizedDepth - u_depthPeekValue) < u_depthPeekTolerance) {
                       finalColor.rgb = vec3(1.0, 0.0, 0.0); // Output pure RED
                    }
                }
...
// ===================================================================
    ...
    // Color Atlas (RGBA) - High Precision for WAA
    ...
    // Depth Atlas - High Precision for WAA accumulation and inpainting.
    infillAtlasTarget_Depth = new THREE.WebGLRenderTarget(canvasWidth, canvasHeight, {
        minFilter: THREE.LinearFilter,
        magFilter: THREE.LinearFilter,
        format: THREE.RGBAFormat, // Must be RGBA for WAA (R=Depth*W, G=W)
        type: THREE.HalfFloatType,
        depthBuffer: false,
        stencilBuffer: false
    });

    // NEW: VTF-Compatible Depth Atlas (Standard 8-bit)
    // Used for the final display mesh displacement (Vertex Texture Fetch).
    infillAtlasTarget_Depth_VTF = new THREE.WebGLRenderTarget(canvasWidth, canvasHeight, {
        minFilter: THREE.LinearFilter,
        magFilter: THREE.LinearFilter,
        format: THREE.RGBAFormat,
        type: THREE.UnsignedByteType, // Standard 8-bit format
        depthBuffer: false,
        stencilBuffer: false
    });
    ...
    // --- NEW: Ground Truth Accumulation Materials (Texture Space WAA) ---
    
    // Material to accumulate COLOR and WEIGHTS using Texture Space WAA.
    groundTruthColorAccumulatorMaterial = new THREE.ShaderMaterial({
        uniforms: {
            tSourceColor: { value: null },  // The original source color texture
            tLayerMask: { value: null },    // layerMaskTarget (Screen Space)
            tUVMap: { value: null }         // uvMapRenderTarget (Screen Space)
        },
        vertexShader: `varying vec2 vUv; void main() { vUv = uv; gl_Position = vec4(position, 1.0); }`,
        fragmentShader: `
            uniform sampler2D tSourceColor;
            uniform sampler2D tLayerMask;
            uniform sampler2D tUVMap;
            varying vec2 vUv; // Screen Space UV
            void main() {
                // Read inputs at the current screen position (vUv)
                float isBG = texture2D(tLayerMask, vUv).g;

                // If the current pixel is background, we want to accumulate it.
                if (isBG > 0.5) {
                    // CRITICAL FIX: Read the Texture UV coordinates from the UV Map
                    vec2 textureUV = texture2D(tUVMap, vUv).rg;

                    // Use the Texture UVs to sample the source color texture
                    vec4 sourceColor = texture2D(tSourceColor, textureUV);

                    // Determine the weight (WAA). Constant weight for now.
                    float weight = 1.0;

                    // Output pre-multiplied color and the weight for additive blending
                    gl_FragColor = vec4(sourceColor.rgb * weight, weight);
                } else {
                    discard;
                }
            }
    ...
    // Material to accumulate DEPTH and WEIGHTS using Texture Space WAA.
    groundTruthDepthAccumulatorMaterial = new THREE.ShaderMaterial({
        uniforms: {
            tSourceDepth: { value: null }, // The original source depth texture
            tLayerMask: { value: null },
            tUVMap: { value: null }
        },
        vertexShader: `varying vec2 vUv; void main() { vUv = uv; gl_Position = vec4(position, 1.0); }`,
        fragmentShader: `
            uniform sampler2D tSourceDepth;
            uniform sampler2D tLayerMask;
            uniform sampler2D tUVMap;
            varying vec2 vUv; // Screen Space UV
            void main() {
                ...
                if (isBG > 0.5) {
                    ...
                    // Use the Texture UVs to sample the source depth texture
                    float sourceDepth = texture2D(tSourceDepth, textureUV).r;

                    // Determine the weight (WAA)
                    float weight = 1.0;

                    // Output pre-multiplied depth (R) and the weight (G).
                    gl_FragColor = vec4(sourceDepth * weight, weight, 0.0, 0.0);
                } else {
                    discard;
                }
            }
    ...
    // NEW: Normalization Material (Required for WAA)
    normalizationMaterial = new THREE.ShaderMaterial({
        uniforms: {
            tAccumulatedData: { value: null },
            u_isDepth: { value: false }
        },
        vertexShader: `varying vec2 vUv; void main() { vUv = uv; gl_Position = vec4(position, 1.0); }`,
        fragmentShader: `
            uniform sampler2D tAccumulatedData;
            uniform bool u_isDepth;
            varying vec2 vUv;

            void main() {
                vec4 accum = texture2D(tAccumulatedData, vUv);
                float weight = 0.0;
                vec3 data = vec3(0.0);

                if (u_isDepth) {
                    // Depth layout (in RGBA target): R=Depth*Weight, G=Weight
                    weight = accum.g;
                    data = vec3(accum.r);
                } else {
                    // Color layout (in RGBA target): RGB=Color*Weight, A=Weight
                    weight = accum.a;
                    data = accum.rgb;
                }

                if (weight > 0.001) {
                    // Normalize data (Data = Data*W / W) and set validity (Alpha) to 1.0
                    gl_FragColor = vec4(data / weight, 1.0);
                } else {
                    // No valid observations, set validity to 0.0
...
depthToColorMaterial = new THREE.ShaderMaterial({
    ...
    // This block (lines 3795-3852) is REPLACED with the function call below:
    initializeInfillAtlasMesh();
}
...
// -----------------------------------------------------------------------------
// --- 2. PATCH: updateCameraAndProjection -------------------------------------
// -----------------------------------------------------------------------------
// (Locate inside updateCameraAndProjection, around line 4080)
// Find the 'for (const layer of mediaLayers)' loop.
// ADD the u_depthPeekActive.value = depthPeekActive; line inside it.
function updateCameraAndProjection() {
    ...
    for (const layer of mediaLayers) {
        if (layer.mesh && layer.mesh.material.uniforms) {
            ...
            // ===================================================================
            // START: BUG FIX (Fix "stuck" red peek)
            // ===================================================================
            uniforms.u_depthPeekActive.value = depthPeekActive; // This line fixes the state
            // ===================================================================
            // END: BUG FIX
            // ===================================================================
    ...
    // ========================================================================
    // CRITICAL FIX: Update Infill Atlas Mesh Uniforms
    // ========================================================================
    // The atlas mesh (even if cloned) is a separate instance and must have its 
    // uniforms updated every frame to ensure its displacement and projection match.
    if (infillAtlasMesh && infillAtlasMesh.material && infillAtlasMesh.material.uniforms) {
        infillAtlasMesh.position.z = portalPlaneWorldZ; // Keep this from original
        const uniforms = infillAtlasMesh.material.uniforms;
        
        // Update Matrices (if used by the specific shader variant)
        // Note: Three.js updates modelViewMatrix before rendering, but custom uniforms need manual updates.
        ...
        if (uniforms.u_cameraPosition) uniforms.u_cameraPosition.value.copy(camera.position);

        // Update Volume Settings (Critical for displacement alignment)
        if (uniforms.u_portalPlaneDepthNorm) uniforms.u_portalPlaneDepthNorm.value = currentNormPortalPlane; // Keep this
        ...
        if (uniforms.u_metricScale) uniforms.u_metricScale.value = metricScaleFactor; // Keep this
        
        // Update other relevant uniforms if necessary (e.g., resolution)
        if (uniforms.u_resolution) uniforms.u_resolution.value.set(renderer.domElement.width, renderer.domElement.height);
    }
    // ========================================================================
...
/**
 * Runs one frame of ground truth accumulation using hardware blending (WAA).
 * MODIFIED: Now requires the UV Map and source textures for Texture Space Accumulation.
 */
function runAccumulationPass(layerMaskTexture, uvMapTexture, sourceColorTexture, sourceDepthTexture) {
    ...
    if (!renderer || !postProcessScene || !postProcessCamera ||
        ...
        !layerMaskTexture || !uvMapTexture || !sourceColorTexture || !sourceDepthTexture)
    {
        return;
    }
    ...
    // 1. Accumulate Color and Weights
    postProcessQuad.material = groundTruthColorAccumulatorMaterial;
    // Set the required uniforms (Updated for Texture Space)
    ...
    groundTruthColorAccumulatorMaterial.uniforms.tUVMap.value = uvMapTexture;
    
    renderer.setRenderTarget(infillAtlasTarget_Color);
    // DO NOT CLEAR.
    renderer.render(postProcessScene, postProcessCamera);

    // 2. Accumulate Depth and Weights
    ...
    groundTruthDepthAccumulatorMaterial.uniforms.tUVMap.value = uvMapTexture;
...
// ===================================================================
// NEW FUNCTION: renderFeedbackOverlay
// Add this new function definition before bakeInfillAtlas (Around line 4300)
// ===================================================================
/**
 * Renders the provided image texture to the screen, overlaying the "Unknown" areas 
 * of the Ground Truth Atlas in red. Used for feedback during Live and Automated sweeps.
 */
function renderFeedbackOverlay(imageTexture) {
    if (!renderer || !postProcessScene || !postProcessCamera || !imageTexture ||
        !feedbackOverlayMaterial || !gapMaskExtractorMaterial || !masterGapTarget ||
        !infillAtlasTarget_Color?.texture) {
        return;
    }

    const postProcessQuad = postProcessScene.children[0];

    // 1. Generate the "Unknown Mask" (1 - Validity/Weight).
    // The WAA normalized output stores validity in Alpha.
    ...
    // 2. Render the final image with the overlay to the screen.
    renderer.setRenderTarget(null);
    renderer.clear();
 ...
 * Includes Normalization (WAA) and VTF compatibility conversion.
 */
async function bakeInfillAtlas() {
    console.log("--- Starting Ground Truth Atlas Bake (WAA + VTF) ---");
    ...
    // Check dependencies
    if (!renderer || !postProcessScene || !postProcessCamera ||
        !infillAtlasTarget_Color?.texture || !infillAtlasTarget_Depth?.texture || !infillAtlasTarget_Depth_VTF ||
        !pullPyramidTargets.length || !pushPyramidTargets.length ||
        !pullMaterial || !pushMaterial || !copyMaterial || !normalizationMaterial ||
        !pingPongRenderTargetA || !pingPongRenderTargetB)
    {
        console.error("Bake (Ground Truth WAA + VTF) failed: Missing essential components.");
        isSweeping = false;
        if (bakeButton) bakeButton.textContent = "Bake Failed";
        return;
    }
    
    // Define standardPPOptions locally
    const standardPPOptions = {
        transparent: false,
        blending: THREE.NoBlending,
        depthTest: false,
    ...
    // ========================================================================
    // STEP 0: NORMALIZATION (WAA Requirement)
    // ========================================================================

    if (bakeButton) bakeButton.textContent = "Baking... (1/4 Normalize)";

    // 0.1 Normalize Color (Output to PingPongA)
    ...
    // 0.2 Normalize Depth (Output to PingPongB)
    ...
    // Define the normalized sources
    const normalizedColorAtlasTexture = pingPongRenderTargetA.texture;
    const normalizedDepthAtlasTexture = pingPongRenderTargetB.texture;


    // --- BAKE 1: DEPTH (High Precision) ---
    console.log("Baking Depth Atlas...");
    if (bakeButton) bakeButton.textContent = "Baking... (2/4 Depth)";

    // 1.1: Initialize Pull Pyramid base for Depth.
    const depthInitMaterial = new THREE.ShaderMaterial({
        uniforms: {
            tDepth: { value: normalizedDepthAtlasTexture },
            tValiditySource: { value: normalizedColorAtlasTexture } // Use Color's alpha for validity
        },
        vertexShader: `varying vec2 vUv; void main() { vUv = uv; gl_Position = vec4(position, 1.0); }`,
        fragmentShader: `
            uniform sampler2D tDepth;
            ...
            void main() {
                ...
                // Also check validity from depth pass (in case it failed)
                float finalValidity = min(validity, texture2D(tDepth, vUv).a);
                gl_FragColor = vec4(vec3(depth), finalValidity);
            }
        `,
        ...standardPPOptions
    });
    ...
    renderer.clear();
    renderer.render(postProcessScene, postProcessCamera);
    depthInitMaterial.dispose(); // Dispose temporary material
    ...
    // 1.5: Copy final result back to the Depth Atlas (High Precision)
    renderer.setViewport(0, 0, renderer.domElement.width, renderer.domElement.height);
    postProcessQuad.material = copyMaterial;
    ...
    // ========================================================================
    // FIX: STEP 1.6: Convert High-Precision Depth to VTF-Compatible Format
    // ========================================================================
    if (bakeButton) bakeButton.textContent = "Baking... (3/4 VTF)";
    // Copy the result from the high-precision target to the 8-bit VTF target.
    postProcessQuad.material = copyMaterial;
    ...
    if (bakeButton) bakeButton.textContent = "Baking... (4/4 Color)";

    // 2.1: Copy the NORMALIZED Color Atlas to the base of the Pull Pyramid.
    postProcessQuad.material = copyMaterial;
    copyMaterial.uniforms.tDiffuse.value = normalizedColorAtlasTexture;
    renderer.setRenderTarget(pullPyramidTargets[0]);
    renderer.render(postProcessScene, postProcessCamera);
    ...
    // --- FINALIZE ---
    // CRITICAL: Update the atlas mesh materials (cloning and setting VTF depth).
    initializeInfillAtlasMesh();

    useStaticInfillAtlas = true;
    isAccumulatingGaps = false;
    isSweeping = false;
    console.log("--- Ground Truth Atlas Bake Complete (WAA + VTF) ---");
 ...
 * MODIFIED for Texture Space WAA and Feedback Overlay.
 */
async function runAutomatedSweep() {
    ...
    // ========================================================================
    // FIX: Ground Truth Initialization (Texture Space)
    // ========================================================================
    // We need the source color and depth textures.
    const firstLayer = mediaLayers.find(l => l.mesh && l.textures.color && l.textures.depth);
    if (!firstLayer) {
        console.error("Sweep failed: No mesh layer with color and depth textures found.");
        // Ensure buttons are reset if we fail here
        const quickBtn = document.getElementById('autoSweepQuickButton');
        if (quickBtn) { quickBtn.disabled = false; quickBtn.textContent = 'Run Quick Bake (Grid)'; }
        return;
    }
    const sourceColorTexture = firstLayer.textures.color;
    const sourceDepthTexture = firstLayer.textures.depth;
    // ========================================================================
    ...
    for (let i = 0; i < steps; i++) {
        for (let j = 0; j < steps; j++) {
            ...
            // Uniforms are disabled inside renderUVMap, but we ensure they are off here too.
            setAllLayerUniforms('u_useDepthGrad', false);
            setAllLayerUniforms('u_useSobel', false);
            setAllLayerUniforms('u_useLuma', false);
            setAllLayerUniforms('u_useChroma', false);
            ...
            // 1.5. NEW: Render UV Map
            renderUVMap();

            // (Locate inside 'runAutomatedSweep' function, in "2. Generate Layer Mask")
            // (Around line 5200)
            // ===================================================================
            // START: BUG FIX (Revert layerMaskMaterial input)
            // ===================================================================
            // 2. Generate Layer Mask (FG/BG Split)
            postProcessQuad.material = layerMaskMaterial;
            // REVERTED: Use the HARDWARE depth from sceneRenderTarget
            ...
            // 3. Run Accumulation Pass (Updated inputs for Texture Space)
            runAccumulationPass(
            ...
            // NEW: Render Feedback Overlay to Screen
            renderFeedbackOverlay(sceneRenderTarget.texture);

            count++;
            if (quickBtn) quickBtn.textContent = `Sweeping... (${count}/25)`;

            // Yield control to allow the renderer to process and the screen to update
            await new Promise(resolve => requestAnimationFrame(resolve)); 
        }
    }
...
async function runContinuousSweep() {
    ...
    for (let frame = 0; frame < totalFrames; frame++) {
        ...
        // (Locate inside 'runContinuousSweep' function, in "2. Generate Layer Mask")
        // (Around line 5310)
        // ===================================================================
        // START: BUG FIX (Revert layerMaskMaterial input)
        // ===================================================================
...
// ===================================================================
// START: REPLACEMENT render() (Texture Space WAA and Overlay Refactor)
// ===================================================================
...
function render() {
    ...
    // CRITICAL FIX: This now also updates the infillAtlasMesh uniforms.
    updateCameraAndProjection();
    ...
    if (useStaticInfillAtlas) {
        ...
        // NOTE: Gap generation on the Atlas Mesh itself is disabled during initialization 
        // (initializeInfillAtlasMesh) and does not need to be explicitly disabled here every frame.
    ...
    // ===================================================================
    // MODIFIED: Helper function for final render passes
    // ===================================================================
    const renderToScreen = (finalImageTexture) => {
        ...
        if (showFeedbackOverlay) {
            // Now apply the overlay using the helper function.
            // It reads from pingPongRenderTargetA.texture (where we just rendered)
            // and the atlas validity mask, and renders to screen.
    ...
    if (debugDepthMaterial) {
        // REMOVED: Linearization uniforms are gone from this material
    ...
    // const hasWireframes = wireframeCubes.length > 0; // Removed from original, re-adding
    const hasRenderableContent = hasMeshLayers || useSolidBackground; // Removed wireframes from check


    // ========================================================================
    // START: BUG FIX (Reliable Source Texture Finding)
    // ========================================================================
    const firstLayer = mediaLayers.find(l => l.mesh && l.mesh.material && l.mesh.material.uniforms);
    let sourceColorTexture = null;
    let sourceDepthTexture = null; // This is the normalized source depth

    if (firstLayer) {
        const uniforms = firstLayer.mesh.material.uniforms;
        // Find Color Texture
        if (uniforms.map) sourceColorTexture = uniforms.map.value; // image
        else if (uniforms.rgbTexture) sourceColorTexture = uniforms.rgbTexture.value; // separated
        else if (uniforms.videoTexture) sourceColorTexture = uniforms.videoTexture.value; // sidebyside
        
        // Find Depth Texture
        if (uniforms.displacementMap) sourceDepthTexture = uniforms.displacementMap.value; // image
        else if (uniforms.depthTexture) sourceDepthTexture = uniforms.depthTexture.value; // separated
        else if (uniforms.videoTexture) sourceDepthTexture = uniforms.videoTexture.value; // sidebyside (it's the same texture)
    }
    ...
    // If features requiring depth are active, ensure depth texture is available.
    ...
    if (requiresDepthFeatures && (!sourceColorTexture || !sourceDepthTexture)) {
        // Cannot proceed without color and depth sources.
        setAllLayerUniforms('u_useDepthGrad', false);
        setAllLayerUniforms('u_useSobel', false);
        setAllLayerUniforms('u_useLuma', false);
        setAllLayerUniforms('u_useChroma', false);
    ...
    // ========================================================================
    // NEW PASS: Render UV Map
    // ========================================================================
    // This must be done while gap methods are still disabled.
    renderUVMap();
    // ========================================================================
    ...
    // --- NEW: Live Sweep Accumulation Logic (Ground Truth WAA) ---
    if (isAccumulatingGaps && !isSweeping) {
        // Run the accumulation pass (Updated inputs for Texture Space).
        runAccumulationPass(
            layerMaskTarget.texture,
            uvMapRenderTarget.texture,
            sourceColorTexture,
    ...
    if (debugView === 'depth') { 
        ...
        // This is correct: use the 8-BIT SOURCE TEXTURE for this view
        debugDepthMaterial.uniforms.tDepth.value = sourceDepthTexture;
        // This is the fix: Force the peek visualization ON for this debug view
    ...
    if (debugView === 'jfa') {
        if (!jfaSeedMaterial || !jfaFloodMaterial || !jfaPingTarget || !jfaPongTarget || !finalEdgeMaskTexture || !sourceDepthTexture || !debugJfaMaterial) { console.error("Missing resources for JFA debug view!"); return; }
        ...
        postProcessQuad.material = jfaSeedMaterial; jfaSeedMaterial.uniforms.tDepth.value = sourceDepthTexture; jfaSeedMaterial.uniforms.u_seedDensity.value = jfaSeedDensity;
    ...
    if (debugView === 'jfa_tolerance') {
         if (!jfaSeedMaterial || !jfaFloodMaterial || !jfaPingTarget || !jfaPongTarget || !finalEdgeMaskTexture || !sourceDepthTexture || !cleanColorTexture || !copyMaterial || !debugJfaToleranceMaterial) { console.error("Missing resources for JFA tolerance debug view!"); return; }
        const jfaEdgeMaskTextureDebug = finalEdgeMaskTexture;
        jfaSeedMaterial.uniforms.tEdgeMask.value = jfaEdgeMaskTextureDebug; jfaSeedMaterial.uniforms.u_maskUsesAlpha.value = maskUsesAlpha;
        postProcessQuad.material = jfaSeedMaterial; jfaSeedMaterial.uniforms.tDepth.value = sourceDepthTexture; jfaSeedMaterial.uniforms.u_seedDensity.value = jfaSeedDensity;
        renderer.setRenderTarget(jfaPingTarget); renderer.clear(); renderer.render(postProcessScene, postProcessCamera);
        ...
        debugJfaToleranceMaterial.uniforms.tOriginalDepth.value = sourceDepthTexture;
    ...
    if (debugView === 'jfa_depth_compare') {
        if (!jfaSeedMaterial || !jfaFloodMaterial || !jfaPingTarget || !jfaPongTarget || !finalEdgeMaskTexture || !sourceDepthTexture || !debugJfaDepthCompareMaterial) { console.error("Missing resources for JFA depth compare debug view!"); return; }
        const jfaEdgeMaskTextureDebug = finalEdgeMaskTexture;
        jfaSeedMaterial.uniforms.tEdgeMask.value = jfaEdgeMaskTextureDebug; jfaSeedMaterial.uniforms.u_maskUsesAlpha.value = maskUsesAlpha;
        postProcessQuad.material = jfaSeedMaterial; jfaSeedMaterial.uniforms.tDepth.value = sourceDepthTexture; jfaSeedMaterial.uniforms.u_seedDensity.value = jfaSeedDensity;
        renderer.setRenderTarget(jfaPingTarget); renderer.clear(); renderer.render(postProcessScene, postProcessCamera);
        ...
        debugJfaDepthCompareMaterial.uniforms.tOriginalDepth.value = sourceDepthTexture;
    ...
    if (debugView === 'pull_coarsest') {
        ...
        for (let i = 1; i <= coarsestIndexDebug; i++) {
            ...
            pullShaderMaterialDebug.uniforms.tFinerDepth.value = sourceDepthTexture;
    ...
    if (debugView === 'fg_inpainted' || debugView === 'bg_inpainted' || debugView === 'fg_layer' || debugView === 'bg_layer') {
        ...
        if (debugView === 'fg_inpainted') {
            ...
            splitMaterial.uniforms.tColor.value = cleanColorTexture;
    ...
    if (!useInpainting && !(isAccumulatingGaps && !isSweeping)) {
        ...
        switch(currentInpaintingMethod) {
            ...
            case 'pullpush':
                ...
                finalCompositeMaterial.uniforms.tOriginal.value = cleanColorTexture;
                renderer.setRenderTarget(finalInpaintedTextureTarget); renderer.clear();
                renderer.render(postProcessScene, postProcessCamera);
                break;

            case 'dilation':
                if (!dilationMaterial || !sourceDepthTexture || !pingPongRenderTargetB?.texture || !pingPongRenderTargetA || !copyMaterial || !finalInpaintedTextureTarget) {
                ...
                for (let i = 0; i < dilationIterations; i++) {
                    ...
                    dilationMaterial.uniforms.tOriginalDepth.value = sourceDepthTexture;
