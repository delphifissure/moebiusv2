    ...
    maskGeneratorMaterial = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                float isUnknown = step(0.5, isGap) * step(0.5, isTargetLayer);
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                if (u_fillKernelSize == 2) {
                    ...
                    if (c1.a > 0.01 && m1 > 0.5) { avgDepth += d1 * c1.a; totalValidAlpha += c1.a; }
                    if (c2.a > 0.01 && m2 > 0.5) { avgDepth += d2 * c2.a; totalValidAlpha += c2.a; }
                    if (c3.a > 0.01 && m3 > 0.5) { avgDepth += d3 * c3.a; totalValidAlpha += c3.a; }
                    if (c4.a > 0.01 && m4 > 0.5) { avgDepth += d4 * c4.a; totalValidAlpha += c4.a; }
                ...
                // --- KERNEL > 2 (NxN) LOGIC ---
                    ...
                    for (float y = start; y <= kernelEnd; y++) {
                        ...
                        // Check if neighbor is valid AND in the correct layer
                            ...
                            if (neighbor.a > 0.01 && mask > 0.5) { // Check if it's in the target layer
...
// NEW: layerMaskMaterial
    ...
    /* --- START FIX: REMOVE this entire block ---
    finalCompositeMaterial = new THREE.ShaderMaterial({
        uniforms: {
            tBackground: { value: null }, // bgInpaintedTarget.texture
            tForeground: { value: null }, // fgInpaintedTarget.texture
            tLayerMask: { value: null }   // layerMaskTarget.texture
        },
        vertexShader: `...`, // (The old shader code)
        fragmentShader: `...` // (The old shader code)
    });
    --- END FIX: REMOVE --- */
...
function updateCameraAndProjection() {
    ...
    // Update legacy mesh just in case (should be harmless)
    // if (octopusMesh && octopusMesh.material.uniforms) {
    // ...
    // }
...
function render() {
    ...
    // This helper function replaces all final render-to-screen passes.
    // It applies the universal dither for debug views.
    ...
    // Update uniforms needed by various post-processing passes
    ...
    // DEPRECATED: legacyEdgeMaskMaterial removed
    ...
    // --- CASE 1: Inpainting Disabled or Debug View Inactive ---
    if (!useInpainting && debugView === 'final') {
        // We are NOT in a debug view, so we can render and exit early.
        
        // 1. SET ALL 6 UNIFORMS for the "Inpainting Off" render
    ...
    // --- CASE 2: Inpainting Enabled OR a Debug View is Active ---
    ...
    // (This is now the only pre-pass, Fidelity Check is removed)
    ...
    // --- PASS: Generate FG/BG Layer Mask (for PullPush) ---
    ...
    // This is now the *only* gap generation method.
    // It's the in-shader method (formerly "legacy").
    
    // 1. SET ALL 6 UNIFORMS from the UI
    setAllLayerUniforms('u_useDepthGrad', document.getElementById('useDepthGradCheck')?.checked || false);
    setAllLayerUniforms('u_useSobel', document.getElementById('useSobelCheck')?.checked || false);
    setAllLayerUniforms('u_useLuma', document.getElementById('useLumaCheck')?.checked || false);
    setAllLayerUniforms('u_useChroma', document.getElementById('useChromaCheck')?.checked || false);
    ...
    if (ditherStrength > 0.01) {
        ...
        // Dither is off, just copy the sharpened result
        postProcessQuad.material = copyMaterial;
        copyMaterial.uniforms.tDiffuse.value = sharpenTarget.texture; // Read from Sharpen pass
    }
