...
function initializeInfillAtlasMesh() {
    ...
    // We clone the primary material to keep lighting/uniforms settings, 
    // but swap the maps for our baked atlas.
    if (!infillAtlasMesh) {
        const geometry = primaryLayer.mesh.geometry.clone();
        const material = primaryLayer.mesh.material.clone();
        ...
        infillAtlasMesh.frustumCulled = false; // Prevent culling issues
        scene.add(infillAtlasMesh);
    }

    const mat = infillAtlasMesh.material;

    // 2. Assign Baked Textures
    ...
    console.log("Infill Atlas Mesh Updated.");
}
...
// ===================================================================
// --- NEW SHADER DEFINITIONS (Texture Space Accumulation) ---
// ===================================================================

// Helper: Vertex Shader for Texture Space Unwrapping
const textureSpaceVertexShader = `
    varying vec2 vUv;
    varying vec4 vScreenPos;

    void main() {
        vUv = uv;
        // UNWRAP: Map UV (0..1) to Clip Space (-1..1) for rendering to texture
        gl_Position = vec4(uv * 2.0 - 1.0, 0.0, 1.0);

        // LOOKUP: Calculate where this vertex currently exists on the user's screen
        // We use the standard model-view-projection to find the screen coordinate
        vec4 worldPos = modelMatrix * vec4(position, 1.0);
        vScreenPos = projectionMatrix * viewMatrix * worldPos;
    }
`;

// Material: Accumulates COLOR + Weight
groundTruthColorAccumulatorMaterial = new THREE.ShaderMaterial({
    uniforms: {
        tSourceColor: { value: null }, // The rendered screen image
        tLayerMask: { value: null },   // Screen-space mask (R=FG)
        u_resolution: { value: new THREE.Vector2(960, 540) }
    },
    vertexShader: textureSpaceVertexShader,
    fragmentShader: `
        ...
        void main() {
            ...
            // 2. Safety: Check if this part of the mesh is actually on screen
            if (abs(ndc.x) > 1.0 || abs(ndc.y) > 1.0 || ndc.z > 1.0 || ndc.z < 0.0) {
                discard; 
            }

            // 3. Map NDC to Screen UV (0..1)
            vec2 screenUV = ndc.xy * 0.5 + 0.5;

            // 4. MASK CHECK: Is the pixel on screen currently Foreground?
            // We only want to bake BACKGROUND.
            float isFG = texture2D(tLayerMask, screenUV).r;
            if (isFG > 0.5) {
                discard; // It's foreground, don't bake it.
            }

            // 5. Sample the Source Color from the Screen
            vec4 sourceColor = texture2D(tSourceColor, screenUV);

            // 6. Accumulate (Weighted Average)
            // Color Layout: RGB = Color * Weight, A = Weight
            float weight = 1.0; 
            
            // Simple edge fading to avoid harsh seams at screen borders
            float edgeDist = min(min(screenUV.x, 1.0 - screenUV.x), min(screenUV.y, 1.0 - screenUV.y));
            weight *= smoothstep(0.0, 0.1, edgeDist);
...
// Material: Accumulates DEPTH + Weight
groundTruthDepthAccumulatorMaterial = new THREE.ShaderMaterial({
    uniforms: {
        tSourceDepth: { value: null }, // The rendered screen depth
        tLayerMask: { value: null },
        u_resolution: { value: new THREE.Vector2(960, 540) }
    },
    vertexShader: textureSpaceVertexShader,
    fragmentShader: `
        ...
        void main() {
            vec3 ndc = vScreenPos.xyz / vScreenPos.w;
            if (abs(ndc.x) > 1.0 || abs(ndc.y) > 1.0 || ndc.z > 1.0 || ndc.z < 0.0) discard;

            vec2 screenUV = ndc.xy * 0.5 + 0.5;

            // MASK CHECK: Strictly ignore Foreground
            float isFG = texture2D(tLayerMask, screenUV).r;
            if (isFG > 0.5) discard;

            float sourceDepth = texture2D(tSourceDepth, screenUV).r;

            // Accumulate
            // Depth Layout: R = Depth * Weight, G = Weight, B/A unused
            float weight = 1.0;
            ...
            gl_FragColor = vec4(sourceDepth * weight, weight, 0.0, 1.0);
        }
    `,
    blending: THREE.AdditiveBlending,
    side: THREE.DoubleSide,
...
function runAccumulationPass(layerMaskTexture, uvMapTexture, sourceColorTexture, sourceDepthTexture) {
    ...
    // We iterate over the actual media layers (Meshes), not a post-process quad.
    const activeLayers = mediaLayers.filter(l => l.mesh && l.mesh.visible);
...
/**
 * Bakes the final Infill Atlas.
 * Normalizes the accumulated WAA buffers, fills holes (where FG was) via Pull-Push,
 * and packs the depth for VTF.
 */
async function bakeInfillAtlas() {
    console.log("--- Starting Ground Truth Atlas Bake (Texture Space) ---");
    isSweeping = true;
    const bakeButton = document.getElementById('manualAccumulationButton');
    
    if (!pullPyramidTargets.length || !pushPyramidTargets.length || !normalizationMaterial) {
    ...
    // --- Helper for Pull-Push ---
    const runPullPush = (sourceTexture, destinationTarget, isDepth) => {
        // 1. Init: Copy Normalized Source to Base Level
        ...
        // 3. Push Phase (Fill Holes)
        // Tip: We copy the coarsest Pull level to the coarsest Push level to start
        postProcessQuad.material = copyMaterial;
    ...
    // --- Step 1: Normalize Accumulation Buffers ---
    if (bakeButton) bakeButton.textContent = "Baking... (Normalize)";
    ...
    renderer.setRenderTarget(pingPongRenderTargetA); // Normalized Color
    renderer.clear();
    renderer.render(postProcessScene, postProcessCamera);
    ...
    renderer.setRenderTarget(pingPongRenderTargetB); // Normalized Depth
    renderer.clear();
    renderer.render(postProcessScene, postProcessCamera);

    // --- Step 2: Inpaint (Pull-Push) ---
    if (bakeButton) bakeButton.textContent = "Baking... (Infilling)";

    // Infill Color -> Final Atlas Target
    runPullPush(pingPongRenderTargetA.texture, infillAtlasTarget_Color, false);

    // Infill Depth -> Final Atlas Target (High Precision)
    runPullPush(pingPongRenderTargetB.texture, infillAtlasTarget_Depth, true);

    // --- Step 3: Convert Depth for VTF (Float -> UnsignedByte) ---
    if (bakeButton) bakeButton.textContent = "Baking... (VTF Pack)";
    
    // We use copyMaterial here because the Depth Texture is already 0-1 (normalized).
    // The render target `infillAtlasTarget_Depth_VTF` is defined as UnsignedByteType,
    // so WebGL will automatically handle the float->byte conversion/clamping during write.
    postProcessQuad.material = copyMaterial;
    ...
    // Re-initialize the mesh to ensure it grabs the new textures
    initializeInfillAtlasMesh();

    console.log("--- Ground Truth Atlas Bake Complete ---");
    if (bakeButton) {
        bakeButton.textContent = 'Start Live Sweep';
        bakeButton.disabled = false;
        bakeButton.style.backgroundColor = '';
    }
}
...
async function runAutomatedSweep() {
    ...
    for (let i = 0; i < steps; i++) {
        for (let j = 0; j < steps; j++) {
            ...
            // Note: We use the hardware depth we just rendered
            ...
            // Corrected input: Using the rendered screen buffers, NOT raw textures
            runAccumulationPass(
                layerMaskTarget.texture,
                null, // UV Map not used
                sceneRenderTarget.texture,      // <--- Screen Color
                sceneRenderTarget.depthTexture  // <--- Screen Depth
            );
...
async function runContinuousSweep() {
    ...
    for (let frame = 0; frame < totalFrames; frame++) {
        ...
        // 3. Run Accumulation Pass
        // Corrected input: Using the rendered screen buffers
        runAccumulationPass(
            layerMaskTarget.texture,
            null,
            sceneRenderTarget.texture,      // <--- Screen Color
