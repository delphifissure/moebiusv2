// A83: relax/seed defaults A/B — harmonic+concentrated (shipped) vs min+sharp (v3.1)
// at the user's troll cam (0.358, 0.002). Real GL (band-mask effects invisible to CPU warp).
...
(async () => {
  ...
  for (const [state, relax, seed] of [['shipped', 'harmonic', '2'], ['v31', 'min', '0']]) {
    ...
    await page.evaluate(({ relax, seed }) => {
      const r = document.getElementById('bgRelaxModeSel'); if (r) r.value = relax;
      const s = document.getElementById('bgSeedModeSel'); if (s) s.value = seed;
      window._noVpScan = true; bgQuickBake = true; window._bgQuickBaked = false; buildBackgroundLayer();
    }, { relax, seed });
    ...
    fs.writeFileSync(path.join(OUTD, 'A94_troll_' + state + '.png'), Buffer.from(png.split(',')[1], 'base64'));
    console.log('wrote A94_troll_' + state + '.png');
    await page.close();
  }
