...
(async () => {
  ...
  for (const [tag, px, py] of POSES) {
    for (const [mode, val] of [['cur', 1.0]]) {
      ...
      fs.writeFileSync(path.join(OUTD, `A112_${ASSET}${NODEQ?'_nodeq':''}_${tag}_${mode}.png`), Buffer.from(png.split(',')[1], 'base64'));
      console.log(`wrote A112_${ASSET}${NODEQ?'_nodeq':''}_${tag}_${mode}.png`);
    }
  }
