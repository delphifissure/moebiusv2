...
fs.copyFileSync(path.join(WT, 'roomImg1.png'), path.join(H, 'defaultImgColor.png'));
fs.copyFileSync(path.join(WT, 'roomDepth1.png'), path.join(H, 'defaultImgDepth.png'));
(async () => {
  ...
  for (const [ptag, px, py] of [['cam0', 0.25, 0.02], ['cam1', -0.25, 0.02]]) {
    ...
    fs.writeFileSync(path.join(OUTD, 'AB_photo_' + ptag + '_valwins.png'), Buffer.from(png.split(',')[1], 'base64'));
    console.log('wrote AB_photo_' + ptag + '_valwins.png');
  }
