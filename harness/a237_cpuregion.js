...
(async () => {
    ...
    const res = await page.evaluate(async (o) => {
        ...
        const rows = [];
        for (const nx of o.nxs) {
            const c = window._plugCpuSweep({ nx, ny: 5, scale: 1 });
            ...
            rows.push({ nx, poses: c.poses, seen: c.nSeen, rev, pin, region: reg, holePerPose: Math.round(c.holeCells / c.poses), ms: c.ms });
        }
        return { pw, ph, N, rows };
    }, { nxs: NXS });
    ...
    for (const r of res.rows) console.log('  ' + String(r.nx).padStart(2) + 'x5 (' + r.poses + ' poses): seen ' + pc(r.seen, res.N) + ' = reveals ' + pc(r.rev, res.N) + ' + pinholes ' + pc(r.pin, res.N) +
