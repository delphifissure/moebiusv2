...
(async () => {
  ...
  page.on('console', m => { const t=m.text(); if (/INK-SEAT|SEAT-DBG|QUICK-BAKE/i.test(t)) console.log('  [pg] '+t.slice(0,120)); });
  ...
  // QUICK BAKE with the ink-seat DISABLED -> bake consumes raw estimator depth (like realtime)
  await page.evaluate(() => { window._noInkSeat = true; window._rayReproject = true; bgQuickBake=true; buildBackgroundLayer(); });
  await new Promise(r => setTimeout(r, 500));
  await shot(0, 0, 'ns_neutral');
  await shot(0.325, 0.06, 'ns_offset');   // party under quick-bake, seat OFF
  await shot(0.42, 0.0, 'ns_far');        // stronger offset to expose any shear/fan
