...
if (ASSET === 'war') {
  fs.copyFileSync(path.join('/workspace/moebiusv2/harness', 'war1000_color.png'), path.join(H, 'defaultImgColor.png'));
  fs.copyFileSync(path.join('/workspace/moebiusv2/harness', 'war1000_depth.png'), path.join(H, 'defaultImgDepth.png'));
} else {
...
(async () => {
  ...
  await page.evaluate((nd) => { if (nd) window._noDequant = true; window._noVpScan = true; bgQuickBake = true; window._bgQuickBaked = false; buildBackgroundLayer(); }, NODEQ);
  ...
  for (const [tag, px, py] of POSES) {
    for (const [mode, val] of [['cur', 1.0]]) {
      ...
      fs.writeFileSync(path.join(OUTD, `A113_${ASSET}${NODEQ?'_nodeq':''}_${tag}_${mode}.png`), Buffer.from(png.split(',')[1], 'base64'));
      console.log(`wrote A113_${ASSET}${NODEQ?'_nodeq':''}_${tag}_${mode}.png`);
    }
  }
