...
(async () => {
  ...
  const res = await page.evaluate(async () => {
    ...
    isSweeping = true; camera.position.x = 0.14; camera.position.y = 0;
    await new Promise(r2 => { let n=0; const tick=()=>{ camera.position.x=0.14; camera.position.y=0; n++; n<6?requestAnimationFrame(tick):r2(); }; requestAnimationFrame(tick); });
