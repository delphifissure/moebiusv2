...
(async () => {
    ...
    const bakeViaUI = async (setSel) => {
        await page.evaluate((o) => { const ids = ['bgPlateFarSel', 'bgPlateFillSel', 'bgPlateMarginSel', 'bgPlateFacesSel', 'bgPlateBandSel', 'bgPlateSkySel', 'bgPlateSeamSel', 'bgPlateJoinSel', 'bgPlateRulesSel'];   // OPTS may give 6, 8 or 9 (seams, join, rules)
            ...
            if (o.change) { const [k, v] = o.change.split('='); const id = { far: 'bgPlateFarSel', fill: 'bgPlateFillSel', margin: 'bgPlateMarginSel', faces: 'bgPlateFacesSel', band: 'bgPlateBandSel', sky: 'bgPlateSkySel', seams: 'bgPlateSeamSel', join: 'bgPlateJoinSel', rules: 'bgPlateRulesSel' }[k]; const el = document.getElementById(id); el.value = v; el.dispatchEvent(new Event('change')); }
