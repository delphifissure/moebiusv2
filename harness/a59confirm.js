...
(async () => {
  ...
  page.on('console', m => { const t=m.text(); if (/plate plugs|ERR/.test(t)) console.log('LOG '+t.slice(0,120)); });
  ...
  console.log('built default');
  const png = await page.evaluate(async () => { const L=mediaLayers[0]; isSweeping=true;
    await new Promise(r2=>{let n=0;const t=()=>{camera.position.set(0,0,0.2);n++;n<6?requestAnimationFrame(t):r2();};requestAnimationFrame(t);});
    L.mesh.visible=false; render(); const p=renderer.domElement.toDataURL('image/png'); L.mesh.visible=true; return p; });
  fs.writeFileSync(OUT+'/default_headplate.png', Buffer.from(png.split(',')[1],'base64')); console.log('wrote default_headplate');
