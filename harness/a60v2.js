...
(async () => {
  ...
  page.on('console', m => { const t=m.text(); if (/full planes|ERR|error/i.test(t)) console.log('  [pg] '+t.slice(0,100)); });
  ...
  const info = await page.evaluate(() => { bgQuickBake=false; bgMPIFullPlanes=true; bgMPIMode=true; buildBackgroundLayer();
    return { rp: (typeof bgRayReproject!=='undefined'?bgRayReproject:'undef'), planes: (typeof mpiFullMeshes!=='undefined'&&mpiFullMeshes)?mpiFullMeshes.length:-1 }; });
  console.log('built v2, reproject-default=' + info.rp + ', planes=' + info.planes);
  ...
  await shot(0, 0, 'v2rp_c');
  await shot(0.42, 0, 'v2rp_r');
