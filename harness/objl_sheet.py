...
for fx, fy in R['poses']:
    key = f'{fx}:{fy}'; tv = os.path.join(d, 'truth_' + str(fx).replace('-', 'm') + '_' + str(fy).replace('-', 'm') + '.png')
    ...
    for a in arms:
        ...
        table.append({'pose': key, 'arm': a, 'holes': hole, 'meanErr': me, 'badFrac': bad})
        tiles.append((app.astype(np.uint8), f'{a}: holes {hole}, mean |err| {me:.1f}, >64: {100*bad:.1f}%'))
...
for r, tiles in enumerate(rows):
    for c, (im, txt) in enumerate(tiles):
        x = pad + c * (Wt + pad); y = 30 + r * (Ht + 22 + pad); dr.text((x, y), txt[:70], fill=(255, 230, 120)); sh.paste(Image.fromarray(im).resize((Wt, Ht), Image.NEAREST), (x, y + 14))
