...
(async () => {
  ...
  const res = await page.evaluate((FLAG) => {
    ...
    const tr = [];
    { const tpx = cx.getImageData(400, 700, 360, 1).data;
      for (let k = 0; k < 360; k += 4) tr.push(+(tpx[k*4]/255).toFixed(3)); }
    const raw = [];
    if (L0._rawDepth && L0._rawDepthW === W) for (const x of [100, 300, 500, 596, 700, 900]) raw.push([x, +L0._rawDepth[300 * W + x].toFixed(3)]);
    return { spans: spans.slice(0, 30), nSpans: spans.length, adoptOnRow: adoptSpans, raw, colProfile, tr };
  }, FLAG);
