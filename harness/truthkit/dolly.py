...
def main():
    ...
    for r in rows:
        ...
        for tag, key, th in (('window 45 deg', 'window', r['theta_window_deg']), ('app head units', 'head', r['theta_head_units_deg']), ('const e (frame widths)', 'conste', r['theta_const_e_deg'])):
            e = D * np.tan(np.radians(th)); Rv = render(prims, plate, (e, 0, D), K=1)
            tiles.append((Image.fromarray(to_u8(np.clip(Rv['rgb'][..., 0, :], 0, 1))), f'{tag}: theta {th:.1f} deg, e {e:.3f} m; phot {r[key + "_extreme_photographed"]:.2f} outp {r[key + "_extreme_outpaint"]:.2f}'))
    sc = 0.5; tiles = [(im.resize((int(im.width * sc), int(im.height * sc))), t) for im, t in tiles]
    Wt, Ht = tiles[0][0].size; pad = 6; cols = 4; rws = (len(tiles) + cols - 1) // cols
