...
//   a70b_wash.png    — the baked bgColorTarget wash readback
...
(async () => {
  ...
  // wash readback
  const wash = null && await page.evaluate(() => {
  ...
  if (wash) fs.writeFileSync('a70b_wash.png', Buffer.from(wash.split(',')[1], 'base64'));
  console.log('wash captured: ' + !!wash);
  for (const [tag, hidePlate] of [['fgoff', true]]) {
    const png = await page.evaluate(async (hidePlate) => {
      mediaLayers[0].mesh.visible = !hidePlate; if (typeof bgLayerMesh !== 'undefined' && bgLayerMesh) bgLayerMesh.visible = true;
      isSweeping = true;
