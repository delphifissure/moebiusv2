// A81: rigidify default-flip A/B — star + troll, off vs on, user poses + wide.
// _noVpScan=true for speed (scan affects SD mask only, not renders).
...
const JOBS = [
  ['star',  path.join(WT, 'starwatcher_color.png'), path.join(WT, 'starwatcher_depth.png'), [[0.182, -0.056], [0.3, 0.0]]],
  ['troll', path.join(WT, 'defaultImgColor.png'),   path.join(WT, 'defaultImgDepth.png'),   [[0.147, 0.008], [0.25, 0.02]]],
];
(async () => {
  ...
  for (const [tag, color, depth, cams] of JOBS) {
    ...
    for (const state of ['off', 'on']) {
      ...
      page.on('console', m => { const t = m.text(); if (/rigidify/.test(t)) console.log('  [pg] ' + t.slice(0, 120)); });
      ...
      await page.evaluate((state) => {
        window._noVpScan = true;
        if (state === 'on') window._enableRigidify = true;
        bgQuickBake = true; window._bgQuickBaked = false; buildBackgroundLayer();
      }, state);
      ...
      for (let ci = 0; ci < cams.length; ci++) {
        ...
        fs.writeFileSync(path.join(OUTD, 'A81_' + tag + ci + '_' + state + '.png'), Buffer.from(png.split(',')[1], 'base64'));
        console.log('wrote A81_' + tag + ci + '_' + state + '.png');
      }
      await page.close();
    }
  }
