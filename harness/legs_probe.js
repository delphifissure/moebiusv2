...
(async () => {
  ...
  await load();
  await page.evaluate(() => { window._dirPlate=true; window._rayReproject=true; bgQuickBake=true; buildBackgroundLayer(); });
  await new Promise(r => setTimeout(r, 400));
  await shot(0.42, 0.02, 'dpON_offset');

  await load();
  await page.evaluate(() => { window._dirPlate=false; window._rayReproject=true; bgQuickBake=true; buildBackgroundLayer(); });
  await new Promise(r => setTimeout(r, 400));
  await shot(0.42, 0.02, 'dpOFF_offset');
