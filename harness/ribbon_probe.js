...
(async () => {
  ...
  for (const [dx, tag] of [[0, 'rb_c'], [0.25, 'rb_r'], [-0.25, 'rb_l']]) {
    const png = await page.evaluate(async (dx) => {
      isSweeping = true;
      await new Promise(r2 => { let n=0; const tick=()=>{ camera.position.set(dx,0.02,0.2); n++; n<6?requestAnimationFrame(tick):r2(); }; requestAnimationFrame(tick); });
      render(); return renderer.domElement.toDataURL('image/png');
    }, dx);
    fs.writeFileSync(OUT+'/'+tag+'.png', Buffer.from(png.split(',')[1],'base64')); console.log('wrote '+tag);
  }
