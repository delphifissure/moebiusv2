...
const MODE = process.argv[3] || 'ground'; // 'ground' (default) or 'pull'
...
(async () => {
  ...
  await page.evaluate((mode) => { if (mode==='pull') window._plugPullPush = true; else if (mode==='ground') window._plugGroundUp = true; bgQuickBake = true; buildBackgroundLayer(); }, MODE);
  ...
  await shot(0.42,0,false, MODE+'_offplate');
  await shot(0.42,0,true,  MODE+'_offcomp');
