...
(async () => {
  ...
  const wash = await page.evaluate(() => {
    ...
    for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
      const s2 = ((H-1-y)*W+x)*4, d = (y*W+x)*4;
      id.data[d]=px[s2]; id.data[d+1]=px[s2+1]; id.data[d+2]=px[s2+2]; id.data[d+3]=255;
    }
    cx.putImageData(id, 0, 0);
    return c.toDataURL('image/png');
  });
  fs.writeFileSync(OUT + '/quick_wash.png', Buffer.from(wash.split(',')[1], 'base64'));
