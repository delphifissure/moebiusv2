...
const ASSET = process.argv[2] || 'war';
if (ASSET === 'war') {
...
const POSES = [['userR', 0.366, 0.008], ['userL', -0.491, -0.030], ['inrange', 0.15, 0.0], ['edge', 0.2, 0.0]];
(async () => {
  ...
  for (const [tag, px, py] of POSES) {
    for (const [mode, val] of [['cur', 1.0]]) {
      ...
      fs.writeFileSync(path.join(OUTD, `A102_${ASSET}_${tag}_${mode}.png`), Buffer.from(png.split(',')[1], 'base64'));
      console.log(`wrote A102_${ASSET}_${tag}_${mode}.png`);
    }
  }
