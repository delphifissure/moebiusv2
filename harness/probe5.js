// Filter-state test: same baked texture, mipmaps off + Linear filters.
...
(async () => {
  ...
  page.on('console', m => { const t=m.text(); if (/probe5|error/i.test(t)) console.log('  [page]', t); });
  ...
  const res = await page.evaluate(async () => {
    ...
    const after = holeCount();
    return { before, after };
  });
  console.log('PROBE5', JSON.stringify(res));
