// A103: star leg-silk attribution at the user's cam — FG vs plate.
...
(async () => {
  ...
  for (const [state, setup] of [['full', ''], ['noplate', 'if (typeof bgLayerMesh !== "undefined" && bgLayerMesh) bgLayerMesh.visible = false; if (typeof bgCardMesh !== "undefined" && bgCardMesh) bgCardMesh.visible = false;'], ['plateonly', 'bgLayerMesh.visible = true; mediaLayers[0].mesh.visible = false;']]) {
    const png = await page.evaluate(async ({ setup }) => {
      eval(setup);
      isSweeping = true;
      await new Promise(r2 => { let n = 0; const tick = () => { camera.position.set(-0.245, -0.035, 0.2); n++; n < 8 ? requestAnimationFrame(tick) : r2(); }; requestAnimationFrame(tick); });
      camera.position.set(-0.245, -0.035, 0.2); render();
      return renderer.domElement.toDataURL('image/png');
    }, { setup });
    fs.writeFileSync(path.join(OUTD, 'A103_' + state + '.png'), Buffer.from(png.split(',')[1], 'base64'));
