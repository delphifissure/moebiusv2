...
const ISL = process.argv[3] !== 'clone'; // default islands; pass 'clone' for old full-clone
...
(async () => {
  ...
  page.on('console', m => { const t=m.text(); if (/RUNG-PLUG|hole-only|CPU plug failed|ERR|Error/.test(t)) console.log('  [pg] '+t.slice(0,140)); });
  ...
  console.log('loaded, building v1 (islands='+ISL+')...');
  await page.evaluate((isl) => {
    window._noBgIslands = !isl;
  ...
  const suf = ISL ? 'isl' : 'clone';
  for (const [tag,dx,dy,fg] of [['v1_'+suf+'_plate',0,0.40,false],['v1_'+suf+'_comp',0,0.40,true]]) {
