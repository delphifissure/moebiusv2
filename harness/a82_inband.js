...
const JOBS = [
  ['star',  path.join(WT, 'starwatcher_color.png'), path.join(WT, 'starwatcher_depth.png'), [[0.14, 0.0], [0.2, 0.0]]],
  ['troll', path.join(WT, 'defaultImgColor.png'),   path.join(WT, 'defaultImgDepth.png'),   [[0.14, 0.0], [0.2, 0.0]]],
];
(async () => {
  ...
  for (const [tag, color, depth, cams] of JOBS) {
    ...
    for (const state of ['off']) {
      ...
      for (let ci = 0; ci < cams.length; ci++) {
        ...
        fs.writeFileSync(path.join(OUTD, 'A82_' + tag + ci + '_' + state + '.png'), Buffer.from(png.split(',')[1], 'base64'));
