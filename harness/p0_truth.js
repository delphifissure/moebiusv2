...
(async () => {
    ...
    page.on('console', m => { const t = m.text(); if (t.includes('A244') || t.includes('A246') || t.includes('A252') || t.includes('A253') || t.includes('A242') || t.includes('A99') || t.includes('a89') || t.includes('A241b') || m.type() === 'error') console.log('  [page:' + m.type() + '] ' + t.slice(0, 600)); });
    ...
    const res = await page.evaluate(async (o) => {
        ...
        const gc0 = o.truth.files.gapClass ? await fetchBin(o.truth.files.gapClass) : null;   // A253: the truth's gap class per hidden texel (1 continuous, 2 interior step, 3 extent)
        const farD = new Float32Array(N), farC = new Uint8Array(N * 3), nearM = new Uint8Array(N), compD = new Float32Array(N), gcT = new Uint8Array(N);
        for (let y = 0; y < ph; y++) for (let x = 0; x < pw; x++) { const i = y * pw + x, j = Math.min(th - 1, Math.floor(y * th / ph)) * tw + Math.min(tw - 1, Math.floor(x * tw / pw));
            farD[i] = farD0[j]; farC[i * 3] = farC0[j * 3]; farC[i * 3 + 1] = farC0[j * 3 + 1]; farC[i * 3 + 2] = farC0[j * 3 + 2]; nearM[i] = nearM0[j]; compD[i] = compD0[j]; if (gc0) gcT[i] = gc0[j]; }
        // the app's shift law
        ...
        const PC = [0, 1, 2, 3].map(() => ({ nR: 0, nBR: 0, sDq: 0, sDqAbs: 0, nFront: 0, nBehind: 0, sCol: 0, sClone: 0 }));   // A253 per truth class (0 = unclassed)
        for (let i = 0; i < N; i++) { const h = nearM[i], r = R[i], b = dis[i]; const pc = PC[gcT[i] & 3];
            if (h) nH++; if (r) nR++; if (b) nB++; if (b && r) nBR++; if (r && !b) nRnotB++; if (b && !h) nBnotH++; if (b && h && !r) nBHnotR++;
            if (r) { const dq = (pS[i] - farD[i]) / q, ds = bgShiftPxAt(lut, pS[i]) - bgShiftPxAt(lut, farD[i]); sDqR += Math.abs(dq); sDsR += Math.abs(ds); pc.nR++;
                ...
                if (b) { sDq += dq; sDqAbs += Math.abs(dq); sDs += Math.abs(ds); if (dq > 0.5) nFront++; else if (dq < -0.5) nBehind++; eDepth[i] = dq;
                    const cl = srcC ? (Math.abs(srcC[i * 4] - farC[i * 3]) + Math.abs(srcC[i * 4 + 1] - farC[i * 3 + 1]) + Math.abs(srcC[i * 4 + 2] - farC[i * 3 + 2])) / 3 : 0;
                    sCol += c / 3; eCol[i] = c / 3; sClone += cl;
                    pc.nBR++; pc.sDq += dq; pc.sDqAbs += Math.abs(dq); if (dq > 0.5) pc.nFront++; else if (dq < -0.5) pc.nBehind++; pc.sCol += c / 3; pc.sClone += cl; } } }
        const perClass = gc0 ? PC.map((p, k) => ({ cls: k, nR: p.nR, nBR: p.nBR, meanDq: p.sDq / Math.max(1, p.nBR), meanAbsDq: p.sDqAbs / Math.max(1, p.nBR), nFront: p.nFront, nBehind: p.nBehind, meanCol: p.sCol / Math.max(1, p.nBR), cloneScale: p.sClone / Math.max(1, p.nBR) })) : null;
        ...
        return { pw, ph, bakeMs, plugFrom, q, poses: poses.length, Rpose: Rpose.slice(0, 8), nH, nR, nB, nBR, nRnotB, nBnotH, nBHnotR, meanDq: sDq / Math.max(1, nBR), meanAbsDq: sDqAbs / Math.max(1, nBR), meanAbsDs: sDs / Math.max(1, nBR), nFront, nBehind,
                 meanAbsDqR: sDqR / Math.max(1, nR), meanAbsDsR: sDsR / Math.max(1, nR), meanCol: sCol / Math.max(1, nBR), meanColR: sColR / Math.max(1, nColR), cloneScale: sClone / Math.max(1, nBR), perClass, geo: window._plugGeoStats || null, depthMap, colMap };
    ...
    try { const SRC = { band: '_qbDisocc', dQ: '_qbDQ', plate: 'PLATE', field: '_geoFarField', obsDepth: '_geoObsDepth', obsCount: '_geoObsCount', lipDeep: '_geoLipDeep', lipNear: '_geoLipNear', lipSpread: '_geoLipSpread', rampDrop: '_geoRampDrop', crossFrac: '_geoCrossFrac', kind: '_geoKind', prov: '_geoProv', cls: '_geoClass', post: '_geoPost', nearMode: '_geoLipNearMode', nearFrac: '_geoLipNearFrac' };   // A252 (one evaluate per array)
    ...
    if (res.perClass) { const nm = ['unclassed', 'CONTINUOUS (ridge on the wall)', 'INTERIOR STEP (calf over thigh)', 'EXTENT (calf over wall)'];
        for (const p of res.perClass) if (p.nR) console.log(`   CLASS ${nm[p.cls]}: R ${p.nR}, B∩R ${p.nBR} (${pct(p.nBR, p.nR)}); depth mean signed ${p.meanDq.toFixed(2)} q, |err| ${p.meanAbsDq.toFixed(2)} q, in front ${p.nFront}, behind ${p.nBehind}; colour ${p.meanCol.toFixed(1)} (clone scale ${p.cloneScale.toFixed(1)})`); }
    if (res.geo && res.geo.obs && res.geo.obs.objects !== undefined) console.log(`   A253: objects ${res.geo.obs.objects}, samples continuous-by-slope ${res.geo.obs.continuous}, interior ${res.geo.obs.interior}, extent ${res.geo.obs.extent}, skirt ${res.geo.obs.skirt}; bound raised ${res.geo.obs.boundRaised}`);
