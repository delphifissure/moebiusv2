...
(async () => {
  ...
  page.on('console', m => { const t=m.text(); if (/MPI-V2|full planes|ERR|error/i.test(t)) console.log('  [pg] '+t.slice(0,110)); });
  ...
  const info = await page.evaluate(() => { bgQuickBake = false; bgMPIFullPlanes = true; bgMPIMode = true; buildBackgroundLayer();
    return { planes: (typeof mpiFullMeshes!=='undefined' && mpiFullMeshes) ? mpiFullMeshes.length : -1 }; });
  console.log('built v2, planes=' + info.planes);
  ...
  await shot(0, 0, 'v2_c');
  await shot(0.42, 0, 'v2_r');
  await shot(0, 0.42, 'v2_d');
