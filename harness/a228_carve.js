...
(async () => {
    ...
    const newPage = async () => {
        ...
        page.on('console', m => { const t = m.text(); if (t.includes('a217') || t.includes('A212 FG pre-tear') || t.includes('A232') || t.includes('A234') || t.includes('A244') || t.includes('A246')) console.log('  [page] ' + t.slice(0, 420)); });
    ...
    const runArm = async (tag, carve) => {
        ...
        const res = await page.evaluate(async (o) => {
            ...
            if (o.v2) { bgQuickBake = false; bgMPIFullPlanes = true; bgMPIMode = true; buildBackgroundLayer(); }   // MODE=v2: the app's default build
            else if (o.geo) { window._plugGeoBand({ flush: !!o.flush, nx: o.nx || undefined, observed: !!o.obs, boundary: !!o.boundary, gateAPriori: !!o.gateA }); }   // A244 (GEO=1): the geometric band
            ...
            return out;
        }, { carve, poses: POSES, z: Z, fs: !!process.env.FS, scan: !!process.env.SCAN, collar: !!process.env.COLLAR, sweep: !!process.env.SWEEP, flush: !!process.env.FLUSH, hole: !!process.env.HOLE, geo: !!process.env.GEO, v2: process.env.MODE === 'v2', obs: !!process.env.OBS, gateA: !!process.env.GATEA, boundary: !!process.env.BOUNDARY, nx: process.env.NX ? parseInt(process.env.NX) : 0, flags: (process.env.FLAGS || '').split(',').filter(Boolean) });
        for (const r of res) {
            fs.writeFileSync(path.join(OUT, tag + (process.env.FS ? '_fs' : '') + (process.env.SCAN ? '_scan' : '') + (process.env.COLLAR ? '_collar' : '') + (process.env.SWEEP ? '_sweep' : '') + (process.env.FLUSH ? '_flush' : '') + (process.env.HOLE ? '_hole' : '') + (process.env.MODE === 'v2' ? '_v2' : '') + (process.env.GEO ? '_geo' : '') + (process.env.OBS ? '_obs' : '') + (process.env.GATEA ? '_gateA' : '') + (process.env.BOUNDARY ? '_bnd' : '') + (process.env.FLAGS ? '_' + process.env.FLAGS.replace(/[^A-Za-z0-9]+/g, '') : '') + '_' + r.name + '.png'), Buffer.from(r.png.split(',')[1], 'base64'));
