...
(async () => {
  ...
  page.on('console', m => { const t=m.text(); if (/plate plugs|ERR|error/i.test(t)) console.log('  [pg] '+t.slice(0,110)); });
  ...
  console.log('built quick-bake');
  const shot = async (dx, dy, reproj, tag) => { const png = await page.evaluate(async ({dx,dy,reproj}) => {
  ...
  }, {dx,dy,reproj}); fs.writeFileSync(OUT+'/'+tag+'.png', Buffer.from(png.split(',')[1],'base64')); console.log('wrote '+tag); };
  // angle, off then on, interleaved so each pair completes together (resilient to timeout)
  const angles = [ ['c',0,0], ['r',0.42,0], ['l',-0.42,0], ['d',0,0.42], ['x',0.32,0.28] ];
  for (const [nm,dx,dy] of angles) {
    await shot(dx,dy,false, 'sw_'+nm+'_off');
    await shot(dx,dy,true,  'sw_'+nm+'_on');
  }
