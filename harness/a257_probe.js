...
(async () => {
    ...
    page.on('console', m => { const t = m.text(); if (/A25[2-7]\]|A246\]|A244\]|\[S3\]|\[S4\]|\[S2[bc]\]/.test(t)) console.log('  [page:log] ' + t.slice(0, 400)); });
    ...
    const meta = await page.evaluate(async (o) => {
        ...
        const sz = window._qbSize; return { pw: sz.pw, ph: sz.ph, outer: outerVolumeDepth, inner: innerVolumeDepth, pn: currentNormPortalPlane, D: Math.abs(camera.position.z - portalPlaneWorldZ), terrariumWidth, terrariumHeight, cloneCount: (typeof window._qbCloneCount === 'number') ? window._qbCloneCount : null };
