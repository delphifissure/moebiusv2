...
(async () => {
  ...
  const setup = await page.evaluate(() => {
    ...
    const sx = Math.round(0.30 * w), sy = Math.round(0.90 * h);
  ...
  const T = { mid: 0, near: -Math.PI/2, far: Math.PI/2 };   // mid FIRST: _dzBase e0 = mid
  for (const lock of [true, false]) {
    ...
    for (const [dtag, tval] of Object.entries(T)) {
      const png = await page.evaluate(async ({ tval, lock }) => {
        ...
        isSweeping = true;
        await new Promise(r2 => { let n = 0; const tick = () => { pin(); camera.position.x = 0.12 * dollyLatGain; camera.position.y = 0.02 * dollyLatGain; n++; n < 10 ? requestAnimationFrame(tick) : r2(); }; requestAnimationFrame(tick); });
        pin(); camera.position.x = 0.12 * dollyLatGain; camera.position.y = 0.02 * dollyLatGain; render();
        window._probeGain = dollyLatGain; return renderer.domElement.toDataURL('image/png');
      }, { tval, lock });
      const tag = 'dq3_' + (lock ? 'lock' : 'free') + '_' + dtag;
      fs.writeFileSync(tag + '.png', Buffer.from(png.split(',')[1], 'base64'));
      const g = await page.evaluate(() => window._probeGain); console.log('wrote ' + tag + '  gain=' + Number(g).toFixed(4));
    }
  }
  await page.evaluate(() => { dollyZoomActive = false; render(); });
