...
(async () => {
  ...
  const m = await p.evaluate(()=>{const D=window._qbDbg; if(!D) return null;
    const {plate,d,pw,ph}=D; let mask=0,fold=0,cells=0; const k=396*(pw/1920);
    for(let y=0;y<ph;y++)for(let x=0;x+1<pw;x++){const i=y*pw+x;
      if(plate[i]<d[i]-0.001)mask++;
      const g=Math.abs(plate[i+1]-plate[i]); cells++; if(g*k>=1)fold++;}
    return {pw,ph,mask:100*mask/(pw*ph),fold:100*fold/cells};});
  console.log(JSON.stringify({tag:TAG,...m}));
