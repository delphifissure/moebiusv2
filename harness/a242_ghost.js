...
const ARM = process.env.FLAGS ? process.env.FLAGS.replace(/[^A-Za-z0-9]+/g, '') : 'wash';
...
(async () => {
    ...
    page.on('console', m => { const t = m.text(); if (t.includes('A242') || t.includes('A215') || t.includes('band fill') || t.includes('A241') || t.includes('NOTE') || m.type() === 'warning' || m.type() === 'error') console.log('  [page:' + m.type() + '] ' + t.slice(0, 300)); });
    ...
    }, { poses: POSES, z: Z, flush: !!process.env.FLUSH, flags: (process.env.FLAGS || '').split(',').filter(Boolean) });
