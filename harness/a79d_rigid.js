...
(async () => {
  ...
  await page.evaluate(() => { window._enableRigidify = true; bgQuickBake = true; window._bgQuickBaked = false; buildBackgroundLayer(); });
  ...
  fs.writeFileSync(path.join(OUTD, 'A79b_rigid.png'), Buffer.from(png.split(',')[1], 'base64'));
  console.log('wrote A79b_rigid.png');
