// A/B strips on/off, same code: rest + pose shots
...
(async () => {
  ...
  await page.evaluate((on) => { bgMPIStrips = on; bgPlugMode='directional'; bgValidMode='auto'; buildBackgroundLayer(); }, ON);
  ...
  const tag = ON ? 'on' : 'off';
  await shot(`sab_${tag}_rest.png`, 0, 0);
  await shot(`sab_${tag}_pose.png`, 0.123, -0.055);
  console.log('done');
